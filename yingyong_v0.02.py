...
class JWGLApp(tk.Tk):
    ...
    def create_widgets(self):
        ...
        self.grades_button = ttk.Button(self.functions_frame, text="查询需重修课程", command=self.show_failed_grades)
        self.enroll_button = ttk.Button(self.functions_frame, text="进入选课抢课", command=self.show_enroll_tab)
    ...
    def update_widgets_state(self):
        try:
            if self.logged_in:
                ...
                self.grades_button.pack(pady=8, padx=10, fill="x")
                self.enroll_button.pack(pady=8, padx=10, fill="x")
    ...
    def _get_current_school_year_term(self):
        """
        辅助函数：根据当前日期判断当前所在的学年和学期。
        返回: (year, term)
        - year: 学年的起始年份（如2024-2025学年返回2024）
        - term: 学期（1 或 2）
        """
        now = datetime.now()
        current_month = now.month
        
        if current_month >= 9:  # 9月及以后，属于下一学年的第一学期
            return now.year, 1
        else:  # 1月到8月，属于本学年的第二学期
            return now.year - 1, 2
    ...
    def fetch_block_courses(self):
        ...
        try:
            # 选课功能通常是为下一学期做准备，这里可以写死为下一学年的第一学期
            # 或者也可以用 _get_current_school_year_term 并做适当调整
            # 为简化逻辑，这里我们查询当前学年的课程
            year, _ = self._get_current_school_year_term()
            term = 1 # 默认为第一学期，如有需要可调整
    ...
    def show_failed_grades(self):
        ...
        # 自动获取当前学年和学期
        year, term = self._get_current_school_year_term()
        # 查询成绩通常是查询刚刚结束的学期，所以如果当前是第一学期（9月后），
        # 应该查询上一学年的第二学期。如果是第二学期（1-8月），则查询本学年的第一学期。
        if term == 1:
            query_year = year - 1
            query_term = 2
        else:
